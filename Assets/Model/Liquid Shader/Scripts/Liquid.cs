...
public class Liquid : MonoBehaviour {
	...
	private float foam;
	private float waveIntensity = 0.15f;
	...
	void Update () {
		...
		wobbleAmt = Vector3.Lerp(wobbleAmt, Vector3.zero, Time.deltaTime * 0.2f);
		...
		wavesMult = Mathf.Lerp(wavesMult, Mathf.Pow(Mathf.Sqrt(wobbleAmt.magnitude * 1.25f) * 0.5f + 1, 2f), Time.deltaTime * 2);
	...
	float VolumeOfMesh(Mesh mesh) {
		...
		for (int i = 0; i < mesh.triangles.Length; i += 3) {
			Vector3 p1 = vertices[triangles[i]]; wobbleAmt.x += lerpedAngVelocity.z * 10f;
	...
	public void TriggerWobble(Vector3 force) {
		wobbleAmt += force;
    }
